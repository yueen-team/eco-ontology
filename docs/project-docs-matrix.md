...
| ADR              | Decision records   | `docs/adr/`                        | present | ADR-0001 through ADR-0007 record repository boundary, cutover, v0.1.0 package, evidence-based promotion, projection governance, bilingual surface plus intake grounding, and the legal-instrument lifecycle plus crosswalk grounding spine. |
| API contracts    | API docs           | `docs/api/README.md`               | present | Indexes schema, projection spec, bundle, receipt, and provenance interfaces.                                                                                                                                                                |
| BDD specs        | Behavior specs     | `specs/README.md`                  | present | First active workflow: `specs/projection-governance-workflow.feature`.                                                                                                                                                                      |
| Verification     | Unified verify     | `verify/verify.mjs`                | present | Exposed through `pnpm verify:all`; includes projection fixtures, release bundle check, receipts, and provenance sidecar generation.                                                                                                         |
...
- Keep `dist/release-bundles/eco-ontology-0.1.0.bundle-manifest.json` current
  through `pnpm release:bundle:update`.
- Keep projection golden/negative fixtures green through
